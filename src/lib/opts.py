...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default='coco',
                             help='coco | kitti | coco_hp | pascal | mydataset')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 10} # 这里将34改为10
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
      if opt.hm_hp:
        opt.heads.update({'hm_hp': 5}) # 这里将17改为5
  ...
  def init(self, args=''):
    ...
    default_dataset_info = {
      'ctdet': {'default_resolution': [512, 512], 'num_classes': 80, 
                ...
                'dataset': 'mydataset'}, # 将coco改为mydataset
      ...
      'multi_pose': {
        ...
        'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
        'dataset': 'mydatset_hp', 'num_joints': 5,
        'flip_idx': [[1, 2], [3, 4]]},
