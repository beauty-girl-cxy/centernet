...
class MultiPoseDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    num_objs = min(len(anns), self.max_objs) # 最多可以加载128个目标
    ...
    if self.split == 'train':
      if not self.opt.not_rand_crop:
        ...
        sf = self.opt.scale # 缩放比例
        cf = self.opt.shift # 应用平移扩增
    ...
    output_res = self.opt.output_res # opt.output_res = max(opt.output_h, opt.output_w)
